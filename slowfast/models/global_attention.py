...
from slowfast.models.causal_utils import (
    build_temporal_causal_mask,
    convert_conv3d_to_causal,
)
...
class GlobalLocalAttention(nn.Module):
    def __init__(
        ...
        if self.causal:
            self.pool_q = convert_conv3d_to_causal(self.pool_q)
            self.pool_k = convert_conv3d_to_causal(self.pool_k)
            self.pool_v = convert_conv3d_to_causal(self.pool_v)
    ...
    def forward(self, x, thw_shape, return_glc=False):
        ...
        if self.causal:
            mask = build_temporal_causal_mask(
                q_shape,
                k_shape,
                self.has_cls_embed,
                self.has_global_embed,
                self.global_embed_num,
                attn.device,
                attn.dtype,
            )
