...
class GLC_Gaze(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        pool_kv_stride_list = list(cfg.MVIT.POOL_KV_STRIDE) if cfg.MVIT.POOL_KV_STRIDE else []
        if cfg.MVIT.POOL_KV_STRIDE_ADAPTIVE is not None:
            ...
            for i in range(cfg.MVIT.DEPTH):
                ...
                pool_kv_stride_list.append([i] + _stride_kv)

        for i in range(len(pool_kv_stride_list)):
            stride_kv[pool_kv_stride_list[i][0]] = pool_kv_stride_list[i][1:]
            if cfg.MVIT.POOL_KVQ_KERNEL is not None:
                pool_kv[pool_kv_stride_list[i][0]] = cfg.MVIT.POOL_KVQ_KERNEL
            else:
                pool_kv[pool_kv_stride_list[i][0]] = [s + 1 if s > 1 else s for s in pool_kv_stride_list[i][1:]]
