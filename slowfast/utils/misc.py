...
def log_model_info(model, cfg, use_train_input=True):
    ...
    # Try to compute FLOPs, but skip if model is not JIT-traceable
    try:
        ...
        logger.warning(f"Could not compute FLOPs (model not JIT-traceable): {type(e).__name__}")
        logger.info("Flops: N/A (skipped)")

    # Try to compute activations, but skip if model is not JIT-traceable
    try:
        activations = get_model_stats(model, cfg, "activation", use_train_input)
        logger.info("Activations: {:,} M".format(activations))
    except (AssertionError, RuntimeError, TypeError) as e:
        logger.warning(f"Could not compute Activations (model not JIT-traceable): {type(e).__name__}")
        logger.info("Activations: N/A (skipped)")
