...
def save_checkpoint(path_to_job, model, optimizer, epoch, cfg, scaler=None, is_best=False):
    """
    Save a checkpoint with smart cleanup strategy.
    Args:
    ...
    path_to_latest = os.path.join(checkpoint_dir, "checkpoint_epoch_latest.pyth")
    with pathmgr.open(path_to_latest, "wb") as f:
        torch.save(checkpoint, f)

    # Save as best checkpoint if needed
    if is_best:
        path_to_best = os.path.join(checkpoint_dir, "checkpoint_epoch_best.pyth")
        with pathmgr.open(path_to_best, "wb") as f:
            torch.save(checkpoint, f)
        logger.info(f"Saved best checkpoint at epoch {epoch + 1}")

    # Cleanup old checkpoints if enabled
    if hasattr(cfg.TRAIN, "CHECKPOINT_KEEP_LAST_N") and cfg.TRAIN.CHECKPOINT_KEEP_LAST_N > 0:
        _cleanup_old_checkpoints(checkpoint_dir, epoch + 1, cfg.TRAIN.CHECKPOINT_KEEP_LAST_N)

    return path_to_checkpoint


def _cleanup_old_checkpoints(checkpoint_dir, current_epoch, keep_last_n):
    """
    Remove old checkpoints, keeping only the last N and special ones (best, latest).
    Args:
    ...
    all_files = pathmgr.ls(checkpoint_dir)
    checkpoint_files = []

    for f in all_files:
        # Skip special checkpoints (best, latest)
        if "best" in f or "latest" in f:
            continue
        # Only process epoch checkpoints
        if "checkpoint_epoch_" in f and f.endswith(".pyth"):
            try:
                # Extract epoch number
                epoch_num = int(f.split("_")[-1].replace(".pyth", ""))
    ...
    for i, (epoch_num, filename) in enumerate(checkpoint_files):
        if i >= keep_last_n:
            path_to_remove = os.path.join(checkpoint_dir, filename)
            try:
                pathmgr.rm(path_to_remove)
